...
gboolean datacontrol_recv_sql_message(GIOChannel *channel,
	...
	if (cond & G_IO_IN) {
		...
		if (nbytes > 0)	{
			...
			buf = (char *) calloc(sizeof(char), nbytes + 1);
			if (buf == NULL) {
				LOGE("Out of memory.");
				return FALSE;
			}
...
static void __send_extra_data(const char *key, const int type, const bundle_keyval_t *kv, void *user_data)
{
	...
	if (_write_socket_pair(fd, &key_len, sizeof(int), &nb) != DATACONTROL_ERROR_NONE) {
		LOGE("Writing a key_len to a file descriptor is failed. errno = %d", errno);
		goto error;
	}

	if (_write_socket_pair(fd, (void *)key, key_len, &nb) != DATACONTROL_ERROR_NONE) {
		LOGE("Writing a key to a file descriptor is failed. errno = %d", errno);
		goto error;
	}

	if (_write_socket_pair(fd, &value_len, sizeof(int), &nb) != DATACONTROL_ERROR_NONE) {
		LOGE("Writing a value_len to a file descriptor is failed. errno = %d", errno);
		goto error;
	}

	if (_write_socket_pair(fd, (void *)value, value_len, &nb) != DATACONTROL_ERROR_NONE) {
		LOGE("Writing a value to a file descriptor is failed. errno = %d", errno);
		goto error;
	}
...
int __datacontrol_send_sql_async(int sockfd, bundle *kb, bundle *extra_kb, datacontrol_request_type type, void *data)
{
	...
	if (_write_socket_pair(sockfd, buf, total_len, &nb) != DATACONTROL_ERROR_NONE) {
...
static int datacontrol_sql_request_provider(datacontrol_h provider, datacontrol_request_type type, bundle *request_data, bundle *extra_kb, int request_id)
{
	...
	do {
		...
		ret = __datacontrol_send_sql_async(socket_info->socket_pair, request_data, extra_kb, type, NULL);
		if (ret != DATACONTROL_ERROR_NONE) {
