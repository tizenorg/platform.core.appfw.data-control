...
static int datacontrol_sql_handle_cb(bundle *b, void *data, resultset_cursor *cursor)
{
	int ret = 0;
...
int __recv_sql_select_process(bundle *kb, int fd, resultset_cursor *cursor)
{
	...
	retval = snprintf(select_map_file, REQUEST_PATH_MAX, "%s%s%s", DATACONTROL_REQUEST_FILE_PREFIX,
...
gboolean datacontrol_recv_sql_message(GIOChannel *channel,
	...
	if (cond & G_IO_IN) {
		...
		if (_read_socket(fd, (char *)&nbytes, sizeof(nbytes), &nb) != DATACONTROL_ERROR_NONE)
			goto error;
		...
		if (nbytes > 0)	{
			...
			if (_read_socket(fd, buf, nbytes, &nb) != DATACONTROL_ERROR_NONE) {
			...
			datacontrol_sql_handle_cb(kb, data, cursor);
		}
...
static int datacontrol_sql_request_provider(datacontrol_h provider, datacontrol_request_type type, bundle *request_data, bundle *extra_kb, int request_id)
{
