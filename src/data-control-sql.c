...
static int datacontrol_sql_handle_cb(bundle *b, void *data)
{
	int ret = 0;
...
error:
	if(((sql_response_cb_s *)data) != NULL) {
		...
		if (sql_dc->request_info_list) {
			...
			g_list_free(sql_dc->request_info_list);
		}
	}
...
static void __send_extra_data(const char *key, const int type, const bundle_keyval_t *kv, void *user_data)
{
	...
	if (_write_socket(fd, &key_len, sizeof(int), &nb) != DATACONTROL_ERROR_NONE) {
		LOGE("Writing a key_len to a file descriptor is failed. errno = %d", errno);
		goto error;
	}

	if (_write_socket(fd, (void *)key, key_len, &nb) != DATACONTROL_ERROR_NONE) {
		LOGE("Writing a key to a file descriptor is failed. errno = %d", errno);
		goto error;
	}

	if (_write_socket(fd, &value_len, sizeof(int), &nb) != DATACONTROL_ERROR_NONE) {
		LOGE("Writing a value_len to a file descriptor is failed. errno = %d", errno);
		goto error;
	}

	if (_write_socket(fd, (void *)value, value_len, &nb) != DATACONTROL_ERROR_NONE) {
		LOGE("Writing a value to a file descriptor is failed. errno = %d", errno);
		goto error;
	}
