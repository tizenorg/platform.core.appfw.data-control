...
EXPORT_API int data_control_bulk_result_data_get_result(data_control_bulk_result_data_h result_data_h, int idx, int *result)
{
	int ret;
	if (result_data_h == NULL) {
		LOGE("Invalid bulk data handle");
	...
	ret = datacontrol_bulk_result_data_get_result(result_data_h, idx, result);
	return ret;
}
