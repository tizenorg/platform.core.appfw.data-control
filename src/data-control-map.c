...
static int __map_data_changed_info_compare_cb(gconstpointer a, gconstpointer b)
{
	map_data_changed_cb_info_s *key1 = (map_data_changed_cb_info_s *)a;
	map_data_changed_cb_info_s *key2 = (map_data_changed_cb_info_s *)b;

	return (strcmp(key1->provider_id, key2->provider_id) || strcmp(key1->data_id, key2->data_id));
}

static void __free_map_data_changed_cb_info(map_data_changed_cb_info_s *info)
{
	if (info->provider_id)
...
static void __noti_map_process(data_control_map_noti_type_e type, GVariant *parameters, gpointer user_data)
{
	...
	datacontrol_h provider;
	GList *find_list;
	map_data_changed_cb_info_s *cb_info = NULL;
	map_data_changed_cb_info_s info;
	...
	find_list = g_list_find_custom(__changed_callback_list, &info,
					(GCompareFunc)__map_data_changed_info_compare_cb);
	if (find_list != NULL) {
		cb_info = (map_data_changed_cb_info_s *)find_list->data;
		...
		gpointer         user_data)
{
	...
	data_control_map_noti_type_e type = DATA_CONTROL_NOTI_MAP_UNDEFINED;
...
int datacontrol_map_register_data_changed_cb(datacontrol_h provider, data_control_map_data_changed_cb callback, void *user_data)
{
	...
	int ret = DATACONTROL_ERROR_NONE;
	map_data_changed_cb_info_s *cb_info = NULL;
	map_data_changed_cb_info_s *removed_cb_info = NULL;
	int monitor_id = 0;
	GList *find_list;
	char *path = NULL;
	...
	path = _get_encoded_path(provider, caller_app_id, DATACONTROL_PATH_TYPE_MAP);
	if (path == NULL) {
	...
	ret = _dbus_signal_init(&monitor_id, path, __handle_map_noti);
	if (ret != DATACONTROL_ERROR_NONE) {
	...
	cb_info = (map_data_changed_cb_info_s *)calloc(1, sizeof(map_data_changed_cb_info_s));
	if (cb_info == NULL) {
		LOGE("map_data_changed_cb_info_s alloc fail out of memory.");
		ret = DATACONTROL_ERROR_OUT_OF_MEMORY;
		goto err;
	}
	...
	if (find_list != NULL) {
		removed_cb_info = (map_data_changed_cb_info_s *)find_list->data;
		__changed_callback_list = g_list_remove(__changed_callback_list, removed_cb_info);
		__free_map_data_changed_cb_info(removed_cb_info);
	}
	__changed_callback_list = g_list_append(__changed_callback_list, cb_info);
	LOGI("datacontrol_map_register_data_changed_cb done for %s", provider_id);
