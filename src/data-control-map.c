...
static int __map_handle_cb(datacontrol_request_s *request_data, void *data, char **value_list, int value_count)
{
	LOGI("__map_handle_cb, request_type: %d", request_data->type);
	...
	if (request_data->type >= DATACONTROL_TYPE_MAP_GET && request_data->type <= DATACONTROL_TYPE_MAP_REMOVE) {
		__map_call_cb(request_data->provider_id, request_data->request_id, request_data->type,
	...
	if (cond & G_IO_IN) {
		...
		if (data_len > 0) {
			...
			if (data) {
				...
				__map_handle_cb(request_data, data, value_list, value_count);
				_free_datacontrol_request(request_data);
