...
static char** datacontrol_map_get_value_list(int fd, int *value_count)
{
	...
	if (_read_socket_pair(fd, (gchar *)&count, sizeof(count), &nb)) {
		LOGE("datacontrol_recv_map_message: ...from %d: EOF\n", fd);
		goto ERROR;
	}
	...
	*value_count = count;
	...
	for (i = 0; i < count; i++) {
		if (_read_socket_pair(fd, (gchar *)&nbytes, sizeof(nbytes), &nb)) {
				LOGE("datacontrol_recv_map_message: ...from %d: EOF\n", fd);
				goto ERROR;
		}

		LOGE("nbytes : %d  %d" , nbytes , nb);
		value_list[i] = (char *) calloc(nbytes + 1, sizeof(char));

		if (_read_socket_pair(fd, value_list[i], nbytes, &nb)) {
			LOGE("datacontrol_recv_map_message: ...from %d: EOF\n", fd);
			goto ERROR;
		}
		LOGE("value_list[i] : %s  %d" , value_list[i] , nb);
	}
...
gboolean datacontrol_recv_map_message(GIOChannel *channel,
	...
	LOGE("datacontrol_recv_map_message: ...from %d:%s%s%s%s\n", fd,
			(cond & G_IO_ERR) ? " ERR" : "",
			(cond & G_IO_HUP) ? " HUP" : "",
			(cond & G_IO_IN)  ? " IN"  : "",
			(cond & G_IO_PRI) ? " PRI" : "");
	...
	if (cond & G_IO_IN) {
		...
		LOGE("datacontrol_recv_map_message: ...from %d: %d bytes\n", fd, nbytes);
		if (nbytes > 0) {
			bundle *kb = NULL;
			...
			if (_read_socket_pair(fd, buf, nbytes, &nb)) {
				free(buf);
				return FALSE;
			}
			...
			LOGE("datacontrol_recv_map_message: ...from %d: OK\n", fd);
			LOGE("datacontrol_recv_map_message: ...caller appid %s: OK\n", bundle_get_val(kb, AUL_K_CALLER_APPID));
...
static int datacontrol_map_request_provider(datacontrol_h provider, datacontrol_request_type type, bundle *request_data, int request_id)
{
	...
	if (socketpair == NULL) {
		...
		do {
			...
			pid = appsvc_run_service(arg_list, request_id, NULL, NULL);
		...
		SECURE_LOGE("consumer socket pair : %d", *socketpair);
		if (*socketpair > 0) {
			...
			if (!gio_read) {
				SECURE_LOGE("Error is %s\n", strerror(errno));
				return DATACONTROL_ERROR_IO_ERROR;
			}

			int g_src_id = g_io_add_watch(gio_read, G_IO_IN | G_IO_HUP,
					datacontrol_recv_map_message, data);

			if (g_src_id == 0) {
				SECURE_LOGE("fail to add watch on socket");
				return DATACONTROL_ERROR_IO_ERROR;
			}
			SECURE_LOGE("Watch on socketpair done.");
		} else {
			SECURE_LOGE("fail to get socket pair");
			return DATACONTROL_ERROR_IO_ERROR;
		}
	}
	...
	return __datacontrol_send_map_async(*socketpair, request_data, type, NULL);
