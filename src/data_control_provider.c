...
EXPORT_API int data_control_provider_add_data_change_noti_consumer_filter_cb(
		data_control_provider_change_noti_consumer_filter_cb callback,
		void *user_data,
		int *callback_id)
{
	if (!callback)
	...
	return datacontrol_provider_add_data_change_noti_consumer_filter_cb(callback, user_data, callback_id);
}

EXPORT_API int data_control_provider_remove_data_change_noti_consumer_filter_cb(int callback_id)
{
	if (callback_id < 1)
		return DATA_CONTROL_ERROR_INVALID_PARAMETER;

	return datacontrol_provider_remove_data_change_noti_consumer_filter_cb(callback_id);
}

EXPORT_API int data_control_provider_send_data_changed_noti(
		...
		bundle *data)
{
	...
	return datacontrol_provider_send_data_changed_noti(
			(datacontrol_h)provider,
			_get_internal_noti_type(type),
			data);
}

EXPORT_API int data_control_provider_foreach_data_changed_noti_consumer_list(
		data_control_h provider,
		...
		void *user_data)
{
	...
	return datacontrol_provider_foreach_data_changed_noti_consumer_list(
