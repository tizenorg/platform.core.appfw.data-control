...
int _write_socket_pair(int fd,
	...
	*bytes_write = 0;
	while (left) {
		LOGI("_write_socket_pair: ...from %d: left %d\n", fd, left);
		nb = write(fd, buffer, left);
		LOGI("_write_socket_pair: ...from %d: nb %d\n", fd, nb);
		if (nb < 0) {
			LOGI("_write_socket_pair: ...write error retry_cnt : %d\n", retry_cnt);
			if (retry_cnt > 0) {
				usleep(10);
				retry_cnt --;
			} else
...
int _read_socket_pair(int fd,
	...
	*bytes_read = 0;
	while (left) {
		LOGI("_read_socket_pair: ...from %d: left %d\n", fd, left);
		nb = read(fd, buffer, left);
		LOGI("_read_socket_pair: ...from %d: nb %d\n", fd, nb);
		if (nb < 0) {
			LOGI("_read_socket_pair: ...read error retry_cnt : %d\n", retry_cnt);
			if (retry_cnt > 0) {
				usleep(10);
				retry_cnt --;
			} else
