...
char *_get_encoded_path(datacontrol_h provider, datacontrol_path_type_e type)
{
	...
	int path_len = strlen(provider->provider_id) + strlen(provider->data_id) + 4;
	...
	snprintf(path, path_len, "%s_%s_%d", provider->provider_id, provider->data_id, type);
	MD5_Init(&mdContext);
