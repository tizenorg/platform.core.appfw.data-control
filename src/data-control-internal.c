...
char *_get_encoded_db_path(char *consumer_appid)
{
	...
	int appid_len = strlen(consumer_appid) + 1;
	char db_path[PATH_MAX];
	char *dup_db_path = NULL;

	MD5_Init(&md_context);
	MD5_Update(&md_context, consumer_appid, appid_len);
	...
	snprintf(db_path, sizeof(db_path), "/run/user/%d/%s%s.db",
			getuid(), DATA_CONTROL_DB_NAME_PREFIX, md5_interface);
...
char *_get_encoded_path(datacontrol_h provider, char *consumer_appid)
{
	...
	MD5_Init(&md_context);
	MD5_Update(&md_context, path, path_len);
	MD5_Final(c, &md_context);
