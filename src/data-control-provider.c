...
static int __send_result(bundle* b, datacontrol_request_type type, void *data) {
	...
	int ret = __datacontrol_send_async(socketpair[1], b, type, data);
...
__datacontrol_handler_cb(bundle *b, int request_id, void* data)
{
	...
	if (socketpair == NULL) {
		...
		socketpair = (int *)calloc(2, sizeof(int));
		...
		LOGE("provider socket pair : %d %d", socketpair[0], socketpair[1]);
		if(socketpair[0] > 0) {
			g_hash_table_insert(__socket_pair_hash, strdup(caller), socketpair);


			GIOChannel *gio_read = NULL;
			gio_read = g_io_channel_unix_new(socketpair[1]);
			close(socketpair[0]);
			if (!gio_read) {
				LOGE("Error is %s\n", strerror(errno));
				return -1;
