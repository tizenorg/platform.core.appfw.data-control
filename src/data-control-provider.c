...
static int __send_select_result(int fd, bundle* b, void* data)
{
	...
	if (sqlite3_step(state) != SQLITE_ROW) {
		...
		if (_write_socket_pair(fd, &column_count, sizeof(int), &nb) != DATACONTROL_ERROR_NONE) {
	...
	for (i = 0; i < column_count; ++i) {
		...
		if (_write_socket_pair(fd, &type, sizeof(int), &nb) != DATACONTROL_ERROR_NONE) {
	...
	for (i = 0; i < column_count; i++) {
		...
		if (column_name == NULL)
			...
			if (_write_socket_pair(fd, &column_name_len, sizeof(int), &nb) != DATACONTROL_ERROR_NONE) {
			...
			if (_write_socket_pair(fd, column_name, column_name_len, &nb) != DATACONTROL_ERROR_NONE) {
	...
	if (_write_socket_pair(fd, &total_len_of_column_names, sizeof(int), &nb) != DATACONTROL_ERROR_NONE) {
	...
	// 6. row count
	if (_write_socket_pair(fd, &row_count, sizeof(int), &nb) != DATACONTROL_ERROR_NONE) {
	...
	do {
		...
		if (offset_idx > offset) {
			...
			for (i = 0; i < column_count; ++ i) {
				...
				if (_write_socket_pair(fd, &size, sizeof(int), &nb) != DATACONTROL_ERROR_NONE) {
				...
				if (size > 0 && size < INT_MAX && !is_null_type) {
					...
					if (_write_socket_pair(fd, value, size, &nb) != DATACONTROL_ERROR_NONE) {
...
static int __send_get_value_result(int fd, bundle *b, void *data)
{
	...
	if (_write_socket_pair(fd, &add_value_count, sizeof(int), &nb) != DATACONTROL_ERROR_NONE) {
	...
	for (i = current_offset; i < add_value_count; i++) {
		...
		if (_write_socket_pair(fd, &length, sizeof(int), &nb) != DATACONTROL_ERROR_NONE) {
			LOGE("Writing a length to a file descriptor is failed. errno = %d", errno);
			return DATACONTROL_ERROR_IO_ERROR;
		}

		LOGI("value_list = %s", value_list[i]);
		if (_write_socket_pair(fd, value_list[i], length, &nb) != DATACONTROL_ERROR_NONE) {
...
static int __send_result(bundle *b, datacontrol_request_type type, void *data)
{
	...
	int ret = __datacontrol_send_async(socket_info->socket_pair, b, type, data);

	LOGI("__datacontrol_send_async result : %d ", ret);

	if (ret == DATACONTROL_ERROR_IO_ERROR) {
