...
static int __send_bulk_insert_result(int fd, void *data)
{
	data_control_bulk_result_data_h bulk_results = (data_control_bulk_result_data_h)data;
	int size = datacontrol_bulk_result_data_get_size(bulk_results);
	int i;
	int result;
	...
	if (_write_socket(fd, &size, sizeof(int), &nb) != DATACONTROL_ERROR_NONE) {
		LOGE("Writing a result size to a file descriptor is failed. errno = %d", errno);
		return DATACONTROL_ERROR_IO_ERROR;
	}
	LOGE("__send_bulk_insert_result %d", size);

	for (i = 0; i < size; i++) {
		result = datacontrol_bulk_result_data_get_result(bulk_results, i);
		...
		result_data = datacontrol_bulk_result_data_get_result_data(bulk_results, i);
		if (result_data != NULL) {
...
int __datacontrol_send_async(int sockfd, bundle *kb, datacontrol_request_type type, void *data)
{
	...
	if (DATACONTROL_TYPE_SQL_SELECT == type)
		...
		ret = __send_bulk_insert_result(sockfd, data);
